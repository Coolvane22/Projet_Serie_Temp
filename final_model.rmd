...
Box.test(serie_wo_ts$resid, lag=12, type="Ljung-Box") # bruit blanc donc stationnarite
```
...
```{r}
## AR
ts_ar <- Arima(y = ts_lm$residuals, order = c(15,0,0)) 
...
On estime à l'ordre 15, car après cet ordre là le PACF devient négligeable. 
### MA
```{r}
ts_ma <- Arima(y = ts_lm$residuals, order = c(0,0,13)) # MA(1)
## Test de blancheur
...
On estime à l'ordre 13, car après cet ordre là l'ACF devient négligeable.
