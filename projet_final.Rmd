...
## On convertit les valeurs des dépenses cumulées en valeurs mensuelles non-cumulées.
for (j in 1:13){
  for (i in 2:12){
...
*Interprétation de l'ACF* : En regardant les auto-corrélations, on remarque des pics fortement positifs (et d'autres négatifs) ainsi qu'un schéma périodique de période 12.  
...
On voit que la relation est fortement positive au décalage 12, ce qui reflète la saisonnalité des données.   
...
## On enleve la tendance (Degré 1 donc differences=1) :
...
### Saisonnalité
```{r}
## On teste différentes valeurs pour la période de la saisonnalité. On teste avec 3, 6, 9 et 
par(mfrow = c(2,2))
...
## On enlève la saisonnalité avec  :
serie_wo_tendance %>% 
  diff(lag=12) %>% 
  ggtsdisplay()
...
# p-val < 0.05 donc pas de bruit blanc : il y a bien quelque chose a modéliser dans la partie non déterministe.

## Stationnarité ?
urca::ur.kpss(ts_lm$residuals) # on accepte h0 : stationnaire ok
...
## Test de blancheur :
Box.test(ts_ar$resid, lag=12, type="Ljung-Box") # bruit blanc donc stationnarité
# On test pour h aussi grand que la saisonnalite
```
*Test de blancheur* : On teste l'hypothèse nulle H0 : Tous les coefficients sont nuls, contre l'hypothèse H1 : Au moins un des coefficients est non-nul. On souhaite être sous H0 pour être en situation de bruit blanc. Le test de Ljung-Box donne une p-value = 0.9998. Cette p-value étant supérieure à 5%, on ne rejette pas H0, nous somme donc en situation de bruit blanc. 
...
# On test pour h aussi grand que la saisonnalité :
```
...
# L'algorithme suggère pour la partie non-saisonnière du modèle p=2, d=1 et q=4. Il ne suggère pas de partie saisonnière. 
...
On transforme les données en serie temporelles, et on affiche les graphiques de l'ACF et des résidus pour valider nos hypotheses :
```{r}
...
On regarde maintenant la structure des résidus : le nuage de point est dispersé, on n'a donc pas de corrélation entres les résidus.
...
pred_arima
```
Les prévisions sont plutôt satisfaisantes. La prévision la plus proche de la réalité reste la prévision avec l'AR(15).
