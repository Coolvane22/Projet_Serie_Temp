...
```
*Test de blancheur* : On teste l'hypothèse nulle H0 tous les coefficients sont nuls contre H1 au moins un des coefficients est non nul. On souhaite être sous H0 pour être en situation de bruit blanc. Le test de Ljung-Box donne une p-value = 0.9998. Cette p-value étant supérieure à 5%, on ne rejette pas H0, nous somme en situation de bruit blanc. 
...
# on test pour h aussi grand que la saisonnalite
```
La p-value du test de Blancheur est égal à 1 (c chelou, pourquoi ?)
...
P-value du test de blancheur inférieure à 5% donc on rejette H0 : on n'est  pas en présence d'un bruit blanc. Ce qui est logique puisqu'un ARIMa avec ordre d > 0 n'est pas un processus stationnaire.
